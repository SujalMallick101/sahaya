...
exports.isStudent = (req, res, next) => {
    try{
        if(req.user.role !== "Registered_User" && req.user.role !== "Admin"){
            return res.status(401).json({ 
                success : false,
                message : "Permission prohibited, only registered users are allowed!"
            })
        }
        // Move to next middleware
        next();
        ...
        return res.status(500).json({
            success : false,
            message : "Something went wrong while checking is registered or not."
        })
    }
}
